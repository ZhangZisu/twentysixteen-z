...
(function($) {
    ...
    $(document)
        .pjax('a', '#page', { fragment: '#page', timeout: 6000 })
		.on('submit', 'form[method="get"]', function(e) {
			e.preventDefault()
			$.pjax.submit(e, '#page', { fragment: '#page', timeout: 6000 })
		})
        .on('pjax:send', function() {
            page.addClass('pjax-loading')
            NProgress.start()
        })
        .on('pjax:complete', function() {
            page.removeClass('pjax-loading')
            NProgress.done()
            Prism.highlightAll()
        })
})(jQuery);
...
if(isMobile) {
	document.getElementById('context-menu').remove()
		...
		(function($) {
		var menu = $('#context-menu'), body = $('body'), w, h, l, t, op = false, fr = true, cl = function() {
			op = false
			menu.hide()
		}
		...
		$(document).contextmenu(function(e) {
			...
			if (op) {
				...
				if ((w + e.clientX) >= $(window).width()) {
					menu.css('left', Math.max(e.pageX - w, 0) + "px")
				} else {
					menu.css('left', e.pageX + "px")
				}
				if ((h + e.clientY) >= $(window).height()) {
					menu.css('top', Math.max(e.pageY - h, 0) + "px")
				} else {
